...
On 25 August 2026, after explicit owner confirmation, Manus remediated a verified browser-debug-collector privacy boundary in the managed workspace and PR #6 source. The former production build loaded a public collector that records browser interactions and request/response data. The collector is now served only by the Vite development server from outside `client/public`; production builds contain no collector reference or collector asset, and the production static server returns `404` for collector script and telemetry routes. Canonical and managed validation passed: frozen install, 15 Vitest files / 30 tests, TypeScript check, production build, development collector availability, and production-bundle exclusion checks. The existing Recharts chunk-size warning remains non-blocking. No data, schema, OAuth, tenancy, merge, or deployment change was performed as part of this remediation.
